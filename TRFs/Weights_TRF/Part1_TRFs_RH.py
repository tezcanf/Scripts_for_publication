...
models_low = 'equal_signal_acoustic+phonemes+surprisal+entropy_low_rh'
          
models_high = 'equal_signal_acoustic+phonemes+surprisal+entropy_high_rh'
...
entropy = np.load(os.path.join(ANOVA_results_path,'spectrogram_RH_entropy_source.npy'), allow_pickle=True)    
langauge = np.load(os.path.join(ANOVA_results_path,'spectrogram_RH_language_source.npy'), allow_pickle=True)     
interaction = np.load(os.path.join(ANOVA_results_path,'spectrogram_RH_interaction_source.npy'), allow_pickle=True)           
utils.line_graph1(Data_French_low["gammatone"].square().sqrt().smooth('source', window_size=0.014, window='gaussian').mean('frequency').mean(
    'source'), Data_French_high["gammatone"].square().sqrt().smooth('source', window_size=0.014, window='gaussian').mean('frequency').mean(
    'source'),Data_Dutch_low["gammatone"].square().sqrt().smooth('source', window_size=0.014, window='gaussian').mean('frequency').mean(
    'source'),Data_Dutch_high["gammatone"].square().sqrt().smooth('source', window_size=0.014, window='gaussian').mean('frequency').mean(
    'source'),['French Low ', 'French High ','Dutch Low ','Dutch High '], 'TRF Spectrogram RH', entropy, langauge,interaction)
