...
result_folder = '/project/3027007.02/Scripts_for_publication/TRFs/Weights_ANOVA/Dutch_part1/'
...
for i in TRF_nos: 
    ...
    for model in models:
        ...
        for subject in SUBJECTS:
            ...
            if i<1:
                rows.append(np.concatenate((trf_lh.h[i].square().sqrt().smooth('source', window_size=0.014, window='gaussian').mean('frequency').x.T,trf_rh.h[i].square().sqrt().smooth('source', window_size=0.014, window='gaussian').mean('frequency').x.T),1))
            else:                                
    ...
    with open(os.path.join(result_folder,'Y_'+TRF_names[i]+'_2_voxel_smoothed_2022_equal_signal_freq_part1_4_stories.pickle'), 'wb') as f:
        pickle.dump(Y, f)
    ...
    for  (ANOVA_test, ANOVA_test_name) in zip(ANOVA_tests,ANOVA_test_names):
        ...
        with open(os.path.join(result_folder,'clu_'+TRF_names[i]+'_'+ANOVA_test_name+'_2_voxel_smoothed_2022_equal_signal_freq_part1_4_stories.pickle'), 'wb') as f:
            pickle.dump(clu, f)
